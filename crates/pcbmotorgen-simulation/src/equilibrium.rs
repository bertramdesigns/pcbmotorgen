...
//! endpoints as COIL-CAPTURE positions anchored to the slot/copper region
...
///
/// `electrical_period_m` is P_e (the slot/electrical pitch = 2 × magnet
/// pitch); `magnet_count` is N ≥ 1.
#[must_use]
...
/// Stable-equilibrium travel envelope anchored to the SLOT/COPPER region.
///
...
/// (independent of N):
///
/// - **min = slot_start + (2/3)·P_e** — first coil captures the first pole
///   at electrical phase 240° (Phase A@120°, Phase B@0°, Phase C@240°).
///   Defaults (slot_start = 30 mm, P_e = 12 mm): 30 + 8 = **38 mm**.
/// - **max = slot_end − (3/4)·P_e** — last coil captured symmetrically at
///   the 270° complementary state. Defaults (slot_end = 177 mm):
///   177 − 9 = **168 mm**.
///
/// `rest_phase_m` is the TRACK-FRAME lattice phase `(slot_start + φ) mod P_e`
/// (φ the baseline rest phase), still N-dependent, so the holding-force
/// chart's zero markers stay aligned to the stable rests. A slot region
/// narrower than the envelope clamps max to min.
#[must_use]
pub fn travel_envelope_over_slots(
    ...
    let min = slot_start_m + (2.0 / 3.0) * period;
    let max = (slot_end_m - (3.0 / 4.0) * period).max(min);
    let phase_track = (slot_start_m + phi) % period;
    TravelEnvelope {
        min_position_m: min,
        max_position_m: max,
...
mod tests {
    ...
    /// Defaults (P_e = 12 mm, slots [30, 177] mm in track coords):
    ...
    /// Slot region narrower than the envelope clamps max to min.
    #[test]
    fn narrow_slot_region_clamps_max_to_min() {
    ...
    #[test]
    fn non_positive_period_zeroes_slot_envelope() {
