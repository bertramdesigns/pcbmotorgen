...
- **Active Area (Copper Region)**: The stator track length populated by active conductor legs ($L_{stator}$ in the equations above). The **routing domain** extends past it by an end **padding** on each side for end turns: $L_{routing} = L_{active} + 2 \cdot \text{padding}$. Never refer to the whole active area as a "slot".
...
- **Travel Envelope**: The span of valid mover positions between the first and last stable rest position inside the copper active area; the input **travel** is the free-travel range the array sweeps (center-to-center), derived from active-area length minus the mover span.
